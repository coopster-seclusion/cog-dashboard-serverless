...
from dependencies import get_wits_client
from models.wits import (
...
from services.wits_client import (
    COMMON_NODES,
    RESERVE_RUN_CLASSES,
    SCHEDULE_METADATA,
    SCHEDULES,
...
def get_schedules(wits: WITSClient = Depends(get_wits_client)):
    try:
...
def get_prices(
    ...
    wits: WITSClient = Depends(get_wits_client),
):
...
@router.get("/prices/spread", response_model=SpreadResponse)
...
@router.get("/quantities/energy", response_model=EnergyQuantitiesResponse)
...
@router.get("/quantities/reserves", response_model=ReserveQuantitiesResponse)
