...
function ForwardTooltip({ slice }: { slice: { points: readonly TooltipPoint[] } }) {
...
export default function ForwardCurve() {
  // Pin to PRSL forward 7 TPs; clear back/from/to so they don't conflict
  ...
  if (!series.length || series[0].data.length === 0) {
    return <ErrorState message="No forward curve data available." />;
  }
    ...
    <div>
      <div className="h-[240px]">
        <ResponsiveLine
          ...
          sliceTooltip={ForwardTooltip as any}
          crosshairType="x"
          animate={false}
        />
      </div>
